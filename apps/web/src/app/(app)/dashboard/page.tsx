...
import type { DashboardDto, DepartmentDto } from '@peoplepay360/shared';
import { getSession } from '@/lib/session';
import { apiGetOrRedirect } from '@/lib/api-client';
import { formatMoney, formatMoneyShort, formatHours } from '@/lib/utils';
...
export default async function DashboardPage({ searchParams }: Props) {
  ...
  // The trend's last bucket is the period the API resolved, so it labels the view.
  const periodLabel = data.monthlyTrend[data.monthlyTrend.length - 1]?.month ?? '';

  return (
    <>
      <PageHeader
        title="Payroll Dashboard"
        subtitle={`Live HR and payroll metrics for ${periodLabel}`}
      />
      ...
      <div className="mt-5 grid grid-cols-2 gap-3 lg:grid-cols-5">
        ...
        <KpiCard
          ...
          tone={
            kpis.attendanceHealth >= 80
              ? 'positive'
              : kpis.attendanceHealth >= 60
                ? 'warning'
                : 'danger'
          }
        />
          ...
          <div className="mb-4 flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-slate-500">
                Items needing attention before payroll finalisation
              </p>
            </div>
          ...
          {totalAlerts === 0 ? (
            ...
            <div className="space-y-2.5">
              ...
              <AlertRow
                ...
                detail={alerts.expiringContracts.slice(0, 3).map((c) => c.name).join(', ')}
      ...
      <div className="mt-5 grid gap-4 lg:grid-cols-2">
        <div className="card p-5">
          <div className="mb-4">
            ...
            <p className="text-xs text-slate-500">
              {attendance.coveragePercent}% coverage · {formatHours(attendance.totalWorkedHours)}{' '}
              worked
            </p>
          </div>
        ...
        <div className="card p-5">
          <h2 className="mb-4 text-sm font-semibold text-slate-900">Department Breakdown</h2>
