...
async function main() {
  ...
  for (const s of schedules) {
    scheduleLinesById[s.id] = s.lines;
  }
  ...
  for (let monthsBack = 2; monthsBack >= 1; monthsBack--) {
    ...
    for (let idx = 0; idx < eligible.length; idx++) {
      ...
      const payslipLines = [
        { code: 'BASIC', name: 'Basic Salary', category: 'BASIC', sequence: 10, amount: basic, quantity: 1, rate: 100 },
        { code: 'HRA', name: 'House Rent Allowance', category: 'ALLOWANCE', sequence: 20, amount: hra, quantity: basic, rate: 40 },
        { code: 'TA', name: 'Transport Allowance', category: 'ALLOWANCE', sequence: 30, amount: ta, quantity: 1, rate: 100 },
        { code: 'MA', name: 'Medical Allowance', category: 'ALLOWANCE', sequence: 40, amount: ma, quantity: 1, rate: 100 },
        ...(ot > 0 ? [{ code: 'OT', name: 'Overtime Pay', category: 'ALLOWANCE', sequence: 50, amount: ot, quantity: overtimeHours, rate: 150 }] : []),
        { code: 'GROSS', name: 'Gross Salary', category: 'GROSS', sequence: 100, amount: gross, quantity: 1, rate: 100 },
        { code: 'PF', name: 'Provident Fund', category: 'DEDUCTION', sequence: 110, amount: pf, quantity: 1, rate: 12 },
        ...(pt > 0 ? [{ code: 'PT', name: 'Professional Tax', category: 'DEDUCTION', sequence: 120, amount: pt, quantity: 1, rate: 100 }] : []),
        ...(tds > 0 ? [{ code: 'TDS', name: 'Income Tax (TDS)', category: 'DEDUCTION', sequence: 130, amount: tds, quantity: 1, rate: 100 }] : []),
        { code: 'NET', name: 'Net Salary', category: 'NET', sequence: 200, amount: net, quantity: 1, rate: 100 },
      ];
