...
export default async function AppLayout({ children }: { children: React.ReactNode }) {
  ...
  // Top navigation mirrors spec B1, filtered by what the role may actually read.
  const items: NavItem[] = [];
  ...
  if (can(role, 'workingSchedules', 'read')) {
    items.push({
      label: 'Configuration',
      href: '/config',
      children: [
  ...
  // Self-service entry point for employees, who have no admin modules.
  if (role === 'EMPLOYEE') {
    items.unshift({ label: 'My Space', href: '/my-space' });
  }
